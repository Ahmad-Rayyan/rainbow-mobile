...
public class WebRTCView extends ViewGroup {
    ...
    private static final String TAG = WebRTCView.class.getName();
    ...
    public WebRTCView(Context context) {
        super(context);
        Log.d(TAG, "WebRTCView Constructor" + this.streamURL);
    ...
    private void cleanSurfaceViewRenderer() {
        Log.d(TAG, "cleanSurfaceViewRenderer");
    ...
    private VideoTrack getVideoTrackForStreamURL(String streamURL) {
        Log.d(TAG, "getVideoTrackForStreamURL streamURL:" + streamURL);
        VideoTrack videoTrack = null;
        if (streamURL != null) {
            Log.d(TAG, "getVideoTrackForStreamURL inside the if");
            ReactContext reactContext = (ReactContext) getContext();
            Webrtc module = reactContext.getNativeModule(Webrtc.class);
            videoTrack = module.getStreamForReactTag(streamURL);
        }
        Log.d(TAG, "getVideoTrackForStreamURL videoTrack:" + videoTrack);
        return videoTrack;
    }
    ...
    private void onFrameResolutionChanged(int videoWidth, int videoHeight, int rotation) {
        ...
        synchronized (layoutSyncRoot) {
            Log.d(TAG, "onFrameResolutionChanged synchronized");
        ...
        if (changed) {
            Log.d(TAG, "onFrameResolutionChanged synchronized changed");
    ...
    private void removeRendererFromVideoTrack() {
        Log.d(TAG, "removeRendererFromVideoTrack");
        if (rendererAttached) {
            ...
            Log.d(TAG, "getVideoTrack get called from removeRendererFromVideoTrack ");
    ...
    private void requestSurfaceViewRendererLayout() {
        Log.d(TAG, "requestSurfaceViewRendererLayout:");
        ...
        if (!invokeIsInLayout()) {
            Log.d(TAG, "requestSurfaceViewRendererLayout: !invokeIsInLayout()");
            onLayout(
                    /* changed */ false,
    ...
    void setStreamURL(String streamURL) {
        Log.d(TAG, "setStreamURL: " + streamURL);
        ...
        if (streamURL == null
            ...
            Log.d(TAG, "getVideoTrackForStreamURL get called");
    ...
    private void setVideoTrack(VideoTrack videoTrack) {
        Log.d(TAG, "setVideoTrack: " + videoTrack);
        VideoTrack oldVideoTrack = this.videoTrack;

        if (oldVideoTrack != videoTrack) {
            Log.d(TAG, "setVideoTrack old not equal new");
            if (oldVideoTrack != null) {
                Log.d(TAG, "oldVideoTrack not equal null");
                if (videoTrack == null) {
                    // If we are not going to render any stream, clean the
                    // surface.
                    Log.d(TAG, "videoTrack equal null");
            ...
            if (videoTrack != null) {
                Log.d(TAG, "setVideoTrack call setRendering");
                tryAddRendererToVideoTrack();
                if (oldVideoTrack == null) {
                    Log.d(TAG, "setVideoTrack cleanSurfaceViewRenderer");
    ...
    private void tryAddRendererToVideoTrack() {
        ...
        if (!rendererAttached
            ...
            Log.d(TAG, "tryAddRendererToVideoTrack rendererAttached called");
            surfaceViewRenderer.init(RainbowSdk.instance().webRTC().getCurrentCall().getEglBaseContext(), rendererEvents);
            videoTrack.addSink(surfaceViewRenderer);
            rendererAttached = true;
            Log.d(TAG, "tryAddRendererToVideoTrack: videoTrack " + videoTrack + " surfaceViewRenderer: " + surfaceViewRenderer + " rendererAttached:" + rendererAttached);
        } else {
            Log.d(TAG, "tryAddRendererToVideoTrack rendererAttached" + rendererAttached + " videoTrack = getVideoTrack() " + getVideoTrack() + "ViewCompat.isAttachedToWindow(this) " + ViewCompat.isAttachedToWindow(this));
        }
    }
}
